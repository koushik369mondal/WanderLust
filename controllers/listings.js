...
// The only function that needs to be changed is module.exports.index
module.exports.index = async (req, res) => {
    // 1. Pagination and Filter Setup
    const { category, search, q } = req.query;
    const listingsPerPage = 12; // Set how many listings to show per page
    ...
    try {
        // 2. Count total documents matching the filter for pagination
        const totalListings = await Listing.countDocuments(filter);
        const totalPages = Math.ceil(totalListings / listingsPerPage);

        // 3. Fetch paginated listings from the database
        ...
        // 4. Badge logic for the current page's listings
        ...
        // 5. Log search queries for analytics (async)
        if (searchQuery) {
            SearchLog.create({
                query: searchQuery,
                resultsCount: totalListings, // Use total count for accuracy
        ...
        // 6. Render the page with all necessary data including pagination
        res.render("listings/index.ejs", { 
            allListings, 
            category: req.query.category,
            searchQuery: searchQuery,
            totalResults: totalListings, // Pass the total count
            hasSearch: !!searchQuery,
            noResults: searchQuery && totalListings === 0,
            // Pass pagination variables to the EJS template
            currentPage,
            totalPages,
        });
        ...
        req.flash("error", "Could not load listings. Please try again later.");
        res.redirect("/");
    }
};
